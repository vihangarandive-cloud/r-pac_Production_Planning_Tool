    ...
    public class MasterDataController : Controller
    {
        ...
        public JsonResult ToggleMachineStatus(string machineCode)
        {
            try
            {
                using (var conn = MSSQLHelper.GetConnection())
                {
                    string currentStatus = conn.ExecuteScalar<string>(
                        "SELECT Status FROM MachineResources WHERE MachineCode = @MachineCode",
                        new { MachineCode = machineCode });

                    string newStatus = currentStatus == "Available" ? "Breakdown" : "Available";

                    Dapper.SqlMapper.Execute(conn,
                        "UPDATE MachineResources SET Status = @Status WHERE MachineCode = @MachineCode",
                        new { Status = newStatus, MachineCode = machineCode });

                    Dapper.SqlMapper.Execute(conn,
                        "UPDATE machine_master SET Status = @Status WHERE MachineCode = @MachineCode",
                        new { Status = newStatus, MachineCode = machineCode });
