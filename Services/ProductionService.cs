    ...
    public class ProductionService
    {
        ...
        public ServiceResult CreateOrder(ProductionOrder order)
        {
            ...
            foreach (var item in shortages)
            {
                if (item.Shortage > 0)
                {
                    return new ServiceResult(false, string.Format(
                        "Insufficient inventory for {0}. Required: {1}, Available: {2}",
                        item.ItemName, item.TotalRequired, item.QuantityOnHand));
                }
            }

            order.CreatedAt = DateTime.Now;
            order.PrePressStatus = "Pending";
